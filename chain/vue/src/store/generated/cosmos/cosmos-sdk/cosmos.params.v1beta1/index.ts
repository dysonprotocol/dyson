...
import { Subspace } from "./module/types/cosmos/params/v1beta1/query"
	...
	actions: {
		...
		async QuerySubspaces({ commit, rootGetters, getters }, { options: { subscribe, all} = { subscribe:false, all:false}, params, query=null }) {
			try {
				const key = params ?? {};
				const queryClient=await initQueryClient(rootGetters)
				let value= (await queryClient.querySubspaces()).data
				...
				throw new Error('QueryClient:QuerySubspaces API Node Unavailable. Could not perform query: ' +  (e.error?.message || e.message))
			}
		},
