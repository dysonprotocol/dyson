	...
	actions: {
		...
		async ServiceConfig({ commit, rootGetters, getters }, { options: { subscribe, all} = { subscribe:false, all:false}, params, query=null }) {
			try {
				...
				let value= (await queryClient.serviceConfig()).data
				...
				throw new Error('QueryClient:ServiceConfig API Node Unavailable. Could not perform query: ' +  (e.error?.message || e.message))
			}
		},
