...
import { DenomOwner } from "./module/types/cosmos/bank/v1beta1/query"
	...
	actions: {
		...
		async QueryAllBalances({ commit, rootGetters, getters }, { options: { subscribe, all} = { subscribe:false, all:false}, params, query=null }) {
			try {
				...
				throw new Error('QueryClient:QueryAllBalances API Node Unavailable. Could not perform query: ' +  (e.error?.message || e.message))
			}
		},
		...
		async QuerySpendableBalances({ commit, rootGetters, getters }, { options: { subscribe, all} = { subscribe:false, all:false}, params, query=null }) {
			try {
				...
				throw new Error('QueryClient:QuerySpendableBalances API Node Unavailable. Could not perform query: ' +  (e.error?.message || e.message))
			}
		},
		...
		async QueryTotalSupply({ commit, rootGetters, getters }, { options: { subscribe, all} = { subscribe:false, all:false}, params, query=null }) {
			try {
				...
				throw new Error('QueryClient:QueryTotalSupply API Node Unavailable. Could not perform query: ' +  (e.error?.message || e.message))
			}
		},
		...
		async QuerySupplyOf({ commit, rootGetters, getters }, { options: { subscribe, all} = { subscribe:false, all:false}, params, query=null }) {
			try {
				const key = params ?? {};
				const queryClient=await initQueryClient(rootGetters)
				let value= (await queryClient.querySupplyOf(query)).data
				
					
				while (all && (<any> value).pagination && (<any> value).pagination.next_key!=null) {
					let next_values=(await queryClient.querySupplyOf({...query, 'pagination.key':(<any> value).pagination.next_key})).data
					value = mergeResults(value, next_values);
				}
				...
				throw new Error('QueryClient:QuerySupplyOf API Node Unavailable. Could not perform query: ' +  (e.error?.message || e.message))
			}
		},
		...
		async QueryDenomMetadata({ commit, rootGetters, getters }, { options: { subscribe, all} = { subscribe:false, all:false}, params, query=null }) {
			try {
				...
				throw new Error('QueryClient:QueryDenomMetadata API Node Unavailable. Could not perform query: ' +  (e.error?.message || e.message))
			}
		},
		...
		async QueryDenomsMetadata({ commit, rootGetters, getters }, { options: { subscribe, all} = { subscribe:false, all:false}, params, query=null }) {
			try {
				...
				throw new Error('QueryClient:QueryDenomsMetadata API Node Unavailable. Could not perform query: ' +  (e.error?.message || e.message))
			}
		},
		...
		async QueryDenomOwners({ commit, rootGetters, getters }, { options: { subscribe, all} = { subscribe:false, all:false}, params, query=null }) {
			try {
				const key = params ?? {};
				const queryClient=await initQueryClient(rootGetters)
				let value= (await queryClient.queryDenomOwners( key.denom, query)).data
				
					
				while (all && (<any> value).pagination && (<any> value).pagination.next_key!=null) {
					let next_values=(await queryClient.queryDenomOwners( key.denom, {...query, 'pagination.key':(<any> value).pagination.next_key})).data
					value = mergeResults(value, next_values);
				}
				...
				throw new Error('QueryClient:QueryDenomOwners API Node Unavailable. Could not perform query: ' +  (e.error?.message || e.message))
			}
		},
		
		
		async sendMsgMultiSend({ rootGetters }, { value, fee = [], memo = '', gas = "200000"  }) {
			try {
