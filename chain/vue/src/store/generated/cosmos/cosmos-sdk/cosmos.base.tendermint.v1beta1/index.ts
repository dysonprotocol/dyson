...
import { ProofOp } from "./module/types/cosmos/base/tendermint/v1beta1/query"
import { ProofOps } from "./module/types/cosmos/base/tendermint/v1beta1/query"
import { Block } from "./module/types/cosmos/base/tendermint/v1beta1/types"
import { Header } from "./module/types/cosmos/base/tendermint/v1beta1/types"
	...
	actions: {
		...
		async ServiceABCIQuery({ commit, rootGetters, getters }, { options: { subscribe, all} = { subscribe:false, all:false}, params, query=null }) {
			try {
				const key = params ?? {};
				const queryClient=await initQueryClient(rootGetters)
				let value= (await queryClient.serviceABCIQuery(query)).data
				
					
				while (all && (<any> value).pagination && (<any> value).pagination.next_key!=null) {
					let next_values=(await queryClient.serviceABCIQuery({...query, 'pagination.key':(<any> value).pagination.next_key})).data
					value = mergeResults(value, next_values);
				}
