...
export const ScheduledRun = {
  encode(message: ScheduledRun, writer: Writer = Writer.create()): Writer {
    if (message.index !== '') {
    ...
    if (message.gasprice !== undefined) {
      DecCoin.encode(message.gasprice, writer.uint32(66).fork()).ldelim()
    }
  ...
  decode(input: Reader | Uint8Array, length?: number): ScheduledRun {
    const reader = input instanceof Uint8Array ? new Reader(input) : input
    let end = length === undefined ? reader.len : reader.pos + length
    const message = { ...baseScheduledRun } as ScheduledRun
  ...
  fromJSON(object: any): ScheduledRun {
    const message = { ...baseScheduledRun } as ScheduledRun
  ...
  toJSON(message: ScheduledRun): unknown {
    const obj: any = {}
    ...
    message.resp !== undefined &&
      (obj.resp = message.resp
        ? MsgRunResponse.toJSON(message.resp)
        : undefined)
  ...
  fromPartial(object: DeepPartial<ScheduledRun>): ScheduledRun {
    const message = { ...baseScheduledRun } as ScheduledRun
    if (object.index !== undefined && object.index !== null) {
