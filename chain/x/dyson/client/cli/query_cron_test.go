...
func networkWithCronObjects(t *testing.T, n int) (*network.Network, []types.Cron) {
	t.Helper()
	...
	return network.New(t, cfg), state.CronList
}

func TestShowCron(t *testing.T) {
	net, objs := networkWithCronObjects(t, 2)
	...
	for _, tc := range []struct {
		...
		t.Run(tc.desc, func(t *testing.T) {
			...
			if tc.err != nil {
				...
				require.NoError(t, err)
				var resp types.QueryGetCronResponse
				require.NoError(t, net.Config.Codec.UnmarshalJSON(out.Bytes(), &resp))
				require.NotNil(t, resp.Cron)
				require.Equal(t,
					nullify.Fill(&tc.obj),
...
func TestListCron(t *testing.T) {
	net, objs := networkWithCronObjects(t, 5)
	...
	t.Run("ByOffset", func(t *testing.T) {
		...
		for i := 0; i < len(objs); i += step {
			...
			out, err := clitestutil.ExecTestCLICmd(ctx, cli.CmdListCron(), args)
			require.NoError(t, err)
			var resp types.QueryAllCronResponse
			require.NoError(t, net.Config.Codec.UnmarshalJSON(out.Bytes(), &resp))
